    ...
    class NumericalsProvider : Provider<object>
    {
        protected override object CreateInstance(IContext context)
        {
            ...
            if (!type.IsNumericType())
                throw new InvalidOperationException("Wrong provider plugged in. Current provider serves Numerical Types instead of " + context.Request.Service.FullName);
        ...
        {
            if (min < byte.MinValue || max-1 > byte.MaxValue)
